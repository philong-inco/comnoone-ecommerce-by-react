...
function DotGiamGiaConfiguration() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setErrors, validateForm }) => {
            ...
            try {
                ...
                let response;
                if (id) {
                    response = await axios.put(`https://weblaptop-by-springboot-and-reactjs.onrender.com/api/v1/discounts/update/${id}`, data);
                    setSnackbar({ open: true, message: 'Đợt giảm giá đã được cập nhật thành công!', severity: 'success' });
                } else {
                    response = await axios.post('https://weblaptop-by-springboot-and-reactjs.onrender.com/api/v1/discounts/add', data);
    ...
    const fetchDggDetail = async () => {
        try {
            debugger
            const response = await axios.get(`https://weblaptop-by-springboot-and-reactjs.onrender.com/api/v1/discounts/${id}`);
            ...
            const sanPhamChiTietData = await Promise.all(
                sanPhamChiTietIds.map(async (idSanPhamChiTiet) => {
                    const spctResponse = await axios.get(`https://weblaptop-by-springboot-and-reactjs.onrender.com/api/san-pham-chi-tiet/get-by-productdetail-id?idProductDetail=${idSanPhamChiTiet}`);
    ...
    const urlFindFilter = 'https://weblaptop-by-springboot-and-reactjs.onrender.com/api/san-pham/find/filter-id?';
