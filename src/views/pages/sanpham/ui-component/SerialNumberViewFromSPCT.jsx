        ...
        <DialogContent>
          <MainCard>
                ...
                <TableBody>
                  {listSeriTemp.map((row, index) => (
                    <TableRow key={row.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      ...
                      <TableCell sx={{ fontWeight: '800', color: '#673AB7' }} align="left" title={`${row.ma.length} ký tự`}>
                        {row.ma}
                      </TableCell>
                      <TableCell align="left">{format(new Date(row.ngayNhap), 'dd-MM-yyyy HH:mm:ss')}</TableCell>
                      {row.trangThai == 0 && (
                        <TableCell align="left">
                          <Chip label="Chưa bán" size="small" sx={{ backgroundColor: '#88C4F5' }} />
                        </TableCell>
                      )}
                      {row.trangThai == 1 && (
                        <TableCell align="left">
                          <Chip label="Đã bán" size="small" sx={{ backgroundColor: '#EDE7F6' }} />
                        </TableCell>
                      )}
                      {row.trangThai == 0 && (
                        <TableCell align="left" title={`Xóa`}>
                          <Button
                            variant="contained"
                            color="error"
                            onClick={() => {
            ...
            <div style={{ display: 'flex' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', width: '70%' }}>
                <div style={{ width: '80%' }}>
                  <DialogContentText id="alert-dialog-slide-description">
                    ...
                    <TextField
                      fullWidth
                      rows={5}
                      onChange={handleWriteSerial}
                      id="serial"
                      label="Thêm serial number cho biến thể này"
                      multiline
                      maxRows={5}
                    />
                  </DialogContentText>
                ...
                <div style={{ width: '25%', margin: '50px 0 0 20px', color: '#FFFFFF' }}>
                  <Button
                    ...
                    variant="contained"
                    color="secondary"
                    sx={{ height: '40px', borderRadius: '7px', marginTop: '0' }}
                  >
                    <IconCheck /> Thêm
                  </Button>
                </div>
              ...
              <div style={{ width: '30%' }}>
                <div>{qr ? <img src={qr} alt="QR Code" style={{ width: '200px', height: '200px' }} /> : <p>Đang tạo mã QR...</p>}</div>
              </div>
            </div>
              ...
              <div style={{ width: '75%' }}>
                ...
                <div style={{ display: 'flex' }}>
                  ...
                  <Button
                    ...
                    sx={{ height: '40px', borderRadius: '7px', marginTop: '20px' }}
                  >
                    <IconReload stroke={2} />
                  </Button>
                </div>
                <div>
                  {openAlbum == true && (
                    <AlbumImageForUpdateSPCT setOpenAlbum={setOpenAlbum} listCurrent={bienThe.listUrlAnhSanPham} setVariant={setBienThe} />
                  )}
                </div>
              </div>
              <div style={{ width: '25%' }}>
            ...
            <div style={{ margin: '100px 0 0 0', textAlign: 'center' }}>
              <Button
                onClick={handleUpdateSPCT}
                variant="contained"
                color="secondary"
                sx={{ height: '60px', borderRadius: '7px', margin: '10px' }}
              >
                <IconCheck /> Lưu lại
              </Button>
              <Button
                onClick={handleClose}
                variant="contained"
                sx={{ height: '60px', borderRadius: '7px', margin: '10px', backgroundColor: '#EDE7F6', color: '#000000' }}
              >
                <IconCheck /> Đóng
              </Button>
            </div>
